...
openerp.gpsmaps = function(instance, local) 
{
    ...
    local.gpsmaps = instance.Widget.extend({
        ...
        odometro: function(item,  map) 
        {
            ...
            var tablero="Datos ";
            if(item["times"]!=undefined)    tablero= tablero + " :: " + item["times"];
            if(item["address"]!=undefined)    tablero= tablero + " :: " + item["address"];
            if(item["event_id"]!=undefined)    tablero= tablero + " :: " + item["event_id"];
            if(item["geofence_id"]!=undefined)    tablero= tablero + " :: " + item["geofence_id"];
        ...
        positions: function(map) 
        {
        	...
        	{        
                ...
                {       
                    ...
                    var obj_positions	= new instance.web
                        .Model('gpsmaps.positions')
                        .query(['address', 'latitude', 'longitude', 'course', 'speed', 'batery', 'milage','times','device_id','event_id'])
                        .group_by(['device_id'])
                        .then(function (results)
                        {      
                            _(results).each(function (positions)
                            {
                                var attributes = positions
                                .query(['address', 'latitude', 'longitude', 'course', 'speed', 'batery', 'milage','times','device_id','event_id'])
                                .filter([['times','>', '12/06/2015 00:00:01']])
                                .order_by('-times')                                
                                .limit(1)
                                .all()                                
                                .then(function (position)
                                {
                                    _(position).each(function (item) 
                                    {
	                                    var vmodelo                                 =String(item["device_id"]);		                

	                                    var modelo                                  =vmodelo.split(",");                                    			                                
	                                    var vehicle_id                              =modelo[0];
	                                    openerp.gpsmaps_var.posicion[vehicle_id]    =item;
                                    });                                  
                                }); 
                            });
                        });
